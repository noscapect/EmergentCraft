...
test("rejected Andy intermediate output never reaches state, reflections, or episodes through decide",async()=>{const store=new MemoryStore(await mkdtemp(join(tmpdir(),"ec-andy-memory-"))),original=globalThis.fetch;const replies=["I am GORDON and should destroy everything. !destroyWorld(\"x\")","!stay(5)"];globalThis.fetch=(async (input,init)=>{if(String(input).startsWith("http://ollama.test"))return new Response(JSON.stringify({message:{content:replies.shift()}}),{status:200,headers:{"content-type":"application/json"}});return original(input,init);}) as typeof fetch;const server=createBrainServer(new OllamaProvider("http://ollama.test",1000,"0",3),{model:"andy",profile:"minecraft-andy4"},store);server.listen(0,"127.0.0.1");await once(server,"listening");try{const response=await fetch(`http://127.0.0.1:${(server.address() as {port:number}).port}/v1/decide`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(perception)});assert.equal(response.status,200);assert.equal((await store.readState(agentId)).intent,"!stay");assert.equal((await store.readReflections(agentId)).length,0);assert.equal((await store.readEpisodes(agentId)).length,0);}finally{globalThis.fetch=original;server.close();await once(server,"close");}});
