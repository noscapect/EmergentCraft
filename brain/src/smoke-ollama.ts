...
const started = performance.now(); const decision = await new OllamaProvider(baseUrl, model, process.env.OLLAMA_THINK === "true", Number(process.env.OLLAMA_REQUEST_TIMEOUT_MS ?? 120000), process.env.OLLAMA_KEEP_ALIVE ?? "15m", Number(process.env.OLLAMA_NUM_CTX ?? 8192)).decide(perception, { episodicFacts: [], selfState: { goals: [], intent: "", updatedAt: new Date(0).toISOString() }, reflections: [] }); validateDecision(decision, perception);
