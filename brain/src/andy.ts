...
export function matchAndyCommand(command:AndyCommand,perception:Perception):CommandMatch {
  ...
  return candidate?{candidate,command}:{error:`No currently offered candidate matches !${command.name}.`};
}
...
export function andyCommandDocs(perception:Perception):string {
  const lines:string[]=[]; const add=(line:string)=>{if(!lines.includes(line))lines.push(line);};
