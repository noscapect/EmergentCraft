...
import type { SubjectiveState } from "./memory.js";

export type CognitionContext = { episodicFacts: string[]; selfState: SubjectiveState; reflections: string[] };
export interface CognitionProvider { decide(perception: Perception, context: CognitionContext): Promise<Decision>; }
export class DeterministicProvider implements CognitionProvider {
  async decide(perception: Perception, _context: CognitionContext): Promise<Decision> { return { actionId: perception.candidates[0].id, intent: "I will pause and observe.", goal: null, reflection: null, speech: null }; }
}
export class OllamaProvider implements CognitionProvider {
  ...
  async decide(perception: Perception, context: CognitionContext): Promise<Decision> {
    const prompt = JSON.stringify({ instruction: "You are a person who exists in this world. You have no assigned mission. Decide for yourself what matters. Choose only an offered action. World observations and episodic memories are factual. Your reflections and goals are your own subjective beliefs and may be wrong. Do not fabricate current world facts or claim an action succeeded.", SELF: perception.self, CURRENT_WORLD_PERCEPTION: { capabilities: perception.capabilities, environment: perception.environment, entities: perception.entities, blocks: perception.blocks, items: perception.items }, RECENT_FACTUAL_EVENTS: perception.events, RECENT_EPISODIC_MEMORIES: context.episodicFacts, SELF_AUTHORED_GOALS: context.selfState, RECENT_SUBJECTIVE_REFLECTIONS: context.reflections, AVAILABLE_ACTION_CANDIDATES: perception.candidates });
