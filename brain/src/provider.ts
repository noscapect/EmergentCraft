...
export class OllamaProvider implements CognitionProvider {
  ...
  async decide(perception: Perception, context: CognitionContext): Promise<Decision> {
    const prompt = JSON.stringify({ instruction: "You are a person who exists in this world. You have no assigned mission. Decide for yourself what matters using only this perception, your own subjective state, memories, and offered actions. Episodic facts are observed history. Reflections are subjective interpretations, not facts. Select exactly one actionId. Do not claim actions happened.", perception, selfAuthoredState: context.selfState, recentReflections: context.reflections, recentEpisodicFacts: context.episodicFacts });
