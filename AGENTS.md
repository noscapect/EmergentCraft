...
- Model provider and profile are separate: Ollama and loopback OpenAI-compatible endpoints support generic/Qwen/Andy profiles. One configured server model serves every resident; it never merges their UUID-isolated memory. Consolidation uses that same model, per agent, in the background and below cognition priority.
