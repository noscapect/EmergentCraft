...
| [Mindcraft](https://github.com/mindcraft-bots/mindcraft) | `5f3acc87b479864124173de444f31fa5538f94a6` | MIT | `profiles/defaults/_default.json`, `src/models/prompter.js`, `src/agent/commands/index.js`, `src/agent/commands/actions.js` | Adapted chat/tool-feedback framing, think-block cleanup, strict command detection/arity/type concepts, and command-document style into `brain/src/andy.ts`, `profiles.ts`, and `provider.ts`. No Mineflayer action, omniscient search, code execution, owner role, survival mode, or goal policy is imported. |
