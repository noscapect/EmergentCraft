...
Candidates are opaque server IDs and retain a server-side `OfferedAction` with exact entity UUID, block position/type, or item UUID/type. `PerceptionBuilder.MAX_CANDIDATES` and Brain `MAX_ACTION_CANDIDATES` are the shared protocol invariant: **40**. The available physical affordances are WAIT, native MOVE_TO/APPROACH, timed and range-checked BREAK_BLOCK, range/capacity-checked PICK_UP_ITEM, and local SPEAK. Every target is checked again when selected and on completion. Navigation always uses Minecraft `PathNavigation`; block destruction uses `Level.destroyBlock` only after native approach, range, hardness, and elapsed-time validation. No action teleports or accepts a model coordinate.

Generic SEARCH_FOR_BLOCK and SEARCH_FOR_ENTITY affordances accept a validated registry identifier and range of at most 48 through the decision contract. The server visits a deterministic inner-to-outer sequence of loaded, dry, standable, native-reachable endpoints and evaluates the target only through the same local LOS perception used for ordinary cognition. It never scans unloaded terrain or exposes coordinates; unavailable, ended, stuck, and timed-out legs advance without their own memory event. One final result returns to cognition without automatic interaction. Float-capable native navigation accepts genuine vertical/physical displacement as progress, while retaining finite stuck and total time limits.
...
The brain uses one server-wide Ollama configuration by default, or `EC_PROVIDER=mock` for deterministic testing. Andy profiles use a bounded `/api/chat` correction loop (default `ANDY_ACTION_ATTEMPTS=3`, hard cap 3): rejected syntax and local read-only tool results are appended to the same chat, while only a final command mapped to an offered candidate becomes a decision. `!stats`, `!inventory`, `!nearbyBlocks`, `!nearbyEntities`, and `!craftable` summarize the immutable perception snapshot only. `!startConversation(name, utterance)` is accepted only for a currently offered nearby SPEAK target; it cannot address arbitrary names. Coordinates match only an already offered travel target within 1.5 blocks. Provider/model/profile are selected in `brain/.env`; all residents share inference capacity while receiving only their own memory context. It binds its own HTTP server to `127.0.0.1`. No model output is evaluated as code, command, filesystem instruction, or direct world operation.
