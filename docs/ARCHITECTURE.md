...
Generic SEARCH_FOR_BLOCK and SEARCH_FOR_ENTITY affordances accept a validated registry identifier and range of at most 48 through the decision contract. The server visits a deterministic, finite sequence of reachable native-navigation legs and evaluates the target only through the same local LOS perception used for ordinary cognition. It never scans unloaded terrain or exposes coordinates; finding something returns to cognition without automatic interaction.
