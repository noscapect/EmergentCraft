...
public final class BrainGateway {
    ...
    public CompletableFuture<Optional<BrainDecision>> decide(AgentPerception perception) {
        ...
        return client.sendAsync(request, HttpResponse.BodyHandlers.ofString()).thenApply(response -> { if (response.statusCode()!=200) { EmergentCraftMod.LOGGER.warn("Brain decision failed for {} epoch={}: HTTP {} {}",perception.name(),perception.epoch(),response.statusCode(),bounded(response.body())); return Optional.<BrainDecision>empty(); } try { BrainDecision decision=GSON.fromJson(response.body(),BrainDecision.class); if(decision==null||decision.actionId()==null) { EmergentCraftMod.LOGGER.warn("Brain decision failed for {} epoch={}: malformed JSON {}",perception.name(),perception.epoch(),bounded(response.body())); return Optional.<BrainDecision>empty(); } return Optional.of(decision); } catch(Exception error) { EmergentCraftMod.LOGGER.warn("Brain decision parse failed for {} epoch={}: {} body={}",perception.name(),perception.epoch(),error.toString(),bounded(response.body())); return Optional.<BrainDecision>empty(); } }).exceptionally(error -> { EmergentCraftMod.LOGGER.warn("Brain request unavailable for {} epoch={}: {}",perception.name(),perception.epoch(),error.toString()); return Optional.<BrainDecision>empty(); });
    }
    ...
    private static String bounded(String value) { if(value==null)return ""; String clean=value.replaceAll("[\\r\\n]+"," "); return clean.length()>800?clean.substring(0,800)+"…":clean; }
}
